...
import { getSmartDEXChainRabbitContract } from 'utils/contractHelpers'
import makeBatchRequest from 'utils/makeBatchRequest'

const smartDEXChainRabbitsContract = getSmartDEXChainRabbitContract()
...
const useGetWalletNfts = () => {
  ...
  useEffect(() => {
    const fetchNfts = async () => {
      try {
        const balanceOf = await smartDEXChainRabbitsContract.methods.balanceOf(account).call()
        ...
        if (balanceOf > 0) {
          ...
          const getTokenIdAndBunnyId = async (index: number) => {
            try {
              const { tokenOfOwnerByIndex, getBunnyId, tokenURI } = smartDEXChainRabbitsContract.methods
